        ...
        while(keep_training):
            ...
            for items in train_loader:
                ...
                # inpaint model
                    ...
                    # train
                    outputs, outputs_merged, gen_loss, dis_loss, logs = self.inpaint_model.process(images, edges, masks)
                ...
                if self.config.SAMPLE_INTERVAL and iteration % self.config.SAMPLE_INTERVAL == 0:
                    for i in range(2):
                        outputs_ = self.postprocess(outputs)[i]
                        edges_   = self.postprocess(1 - edges)[i]
                        masks_   = self.postprocess(images * (1 - masks) + masks)[i]
                        images_  = self.postprocess(images)[i]
                        imsave(outputs_, os.path.join(self.samples_path, 'output.png'))                        
                        imsave(edges_,   os.path.join(self.samples_path, 'edge.png'))
                        imsave(masks_,  os.path.join(self.samples_path, 'mask.png'))
                        imsave(images_,  os.path.join(self.samples_path, 'image.png'))
    ...
    def test(self):
        ...
        for items in tqdm(test_loader):
            ...
            if model == 1:
                ...
                # outputs = torch.zeros(masks.shape).to('cuda')
                # outputs[:, :, 1:-1, 1:-1] = self.edge_model(images_gray, edges, masks)
                outputs = self.edge_model(images_gray, edges, masks)

                # import pdb; pdb.set_trace()
                outputs_merged = (outputs * masks) + (edges * (1 - masks))
                for mi in range(outputs_merged.shape[0]):
                    out_m = outputs_merged[mi]
                    img = images[mi]
                    mask = masks[mi]
#                   save_image(masks[mi], 'test_' +str(mi) +'.png')
                    save_image(img, './datasets/imgs/input_'+ str(index) + '_'+ str(mi) +'.png')
                    save_image(mask, './datasets/masks/input_'+ str(index) + '_'+ str(mi) +'.png')
                    save_image(out_m, './datasets/edges_m/input_'+ str(index) + '_'+ str(mi) +'.png')
                # image1_numpy = image1_tensor.numpy()

            # inpaint model
            elif model == 2:
                first_out, second_out = self.inpaint_model(images, edges, masks)
                outputs_merged = (second_out * masks) + (images * (1 - masks))
